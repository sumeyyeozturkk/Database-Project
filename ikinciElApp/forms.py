...
class AddressForm(forms.ModelForm):
	...
	class Meta:
		model = Address
		fields = ('city','street', 'neighborhood', 'gate_no')
...
class UserProfile(forms.ModelForm):
	...
	class  Meta:
		model = Profile
		adress_id = AddressForm()
		fields = ('name','last_name', 'e_mail', 'phone_number', 'gender', 'birthdate','address_id')
...
class AddProductForm(forms.Form):
	...
	product_price=forms.RegexField(regex=r'^\d+(.\d{1,2})?$', widget=forms.TextInput(attrs=dict(required=True,placeholder='Price')), error_messages = {'invalid': _("Not a valid web address1.")})
